import { auth, clerkClient, User as ClerkUser } from "@clerk/tanstack-react-start/server"
import { redirect } from "@tanstack/react-router"
import { createServerFn } from "@tanstack/react-start"
import { userRepository } from "@/db/repositories/user.repository"
import type { NewUser, User as DBUser } from "@/db/schemas/user.schema"
...
export const ensureUser = createServerFn({ method: 'GET' }).handler(withLogging("Ensure User", async () => {
    ...
    try {
        ...
        throw new Error(`Failed to check existing user: ${error instanceof Error ? error.message : 'Unknown error'}`)
    }
    ...
    try {
        ...
        throw new Error(`Failed to fetch Clerk user: ${error instanceof Error ? error.message : 'Unknown error'}`)
    }
    ...
    const primaryEmail: string | undefined = validatedClerkUser.email_addresses.find(
    ...
    const name: string = [validatedClerkUser.first_name, validatedClerkUser.last_name].join(' ')
