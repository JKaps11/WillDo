...
export default function PriorityBadge({ priority, className }: PriorityBadgeProps): ReactNode {
    const level = priorityLevels[priority];
    const activeColor = priorityColors[priority];

    return (
        <div className={cn('flex items-end gap-0.5 h-4', className)} aria-label={`Priority: ${priority.replace('_', ' ')}`}>
            {[1, 2, 3, 4, 5].map((bar) => (
                <div
                    key={bar}
                    className={cn(
