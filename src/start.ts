 ...
 */
const wideEventMiddleware: AnyRequestMiddleware = createMiddleware({ type: 'request' }).server(
    async ({ request, next }) => {
        // Dynamic import to prevent bundling into client
        const { runWithWideEvent, getWideEvent } = await import('./lib/logging/wideEventStore.server')
 ...
 */
const enrichUserMiddleware: AnyRequestMiddleware = createMiddleware({ type: 'request' }).server(
    async ({ next }) => {
        const result = await next()
